...
makeMatrix <- function(x = matrix()) {
        ...
        ## the makeMatrix function returns 4 other functions
}
...
cacheSolve <- function(x=matrix(), ...) {
        ...
        ## The input matrix x was created with makeMatrix and also carries
        ...
        i <- x$getinverse()
        ## within cacheSolve, set i to be i from makeMatrix, if it exists
        if(!is.null(i)) {
        ...
        data <- x$get()
        ## data is assigned the matrix x from makeMatrix
        i <- solve(data, ...)
